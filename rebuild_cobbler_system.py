...
for k,v in lis.items():
    ...
    print 'Done => %s' %cmd
    ipmi_ip = v['ipmi']
    cmd = 'cobbler system  add --name=%s --hostname=%s --profile=Centos7-cn0.ketong.com --power-type=ipmilan --power-user=root --power-pass=calvin --power-address=%s'  %(
hostname, hostname, ipmi_ip)
    os.system(cmd)
    print 'Done => %s' %cmd
